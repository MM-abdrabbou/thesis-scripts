...
###############################################################################################################################################
#																																  		      #
# Program for extracting compressed FASTQ files, performing alignment using soap2 on adapter-trimmed and quality-filtered reads,		      #
...
REF_GENOME=/media/rna/INIA/benchmark/Alignment/Soap/reference/DM_1-3_516_R44_potato_genome_assembly.v6.1.fa.index
...
OUT_PATH_BAM=/media/rna/INIA/benchmark/Alignment/Soap/Soap_BAM
mkdir -p $OUT_PATH_BAM
# Creating and specifying the directory for the alignment statistics
OUT_PATH_STAT=/media/rna/INIA/benchmark/Alignment/Soap/Soap_STAT
...
for folder in $IN_PATH/*; do
	...
	INIT_BAM_Unpaired=${OUT_PATH_BAM}/${sample}/${sample}_unpaired.bam
	SORT_BAM=${OUT_PATH_BAM}/${sample}/${sample}_sorted.bam
	SORT_BAM_Unpaired=${OUT_PATH_BAM}/${sample}/${sample}_unpaired_sorted.bam
	...
	unzip_start=`date +%s`
	for comp in ${folder}/*qualtrim.fastq.bz2; do
		bzip2 -dk $comp
	done
	#bzip2 -dk $folder
	unzip_end=`date +%s`
	...
	/media/rna/INIA/benchmark/Scripts/soap2.21release/soap -D $REF_GENOME -a $fastq1 -b $fastq2  -o $INIT_BAM -2 $INIT_BAM_Unpaired
	#samtools view -@ $NTHREADS $INIT_BAM -ub - > $INIT_BAM
	#samtools view -@ $NTHREADS $INIT_BAM_Unpaired -ub - > $INIT_BAM_Unpaired
	align_end=`date +%s`
	align_time=$((align_end-align_start))
	echo "SOAP2 time in s:	$align_time" >> $COMP
	...
	samtools flagstat -@ $NTHREADS $INIT_BAM_Unpaired > ${OUT_PATH_STAT}/${sample}/flagstat_stats_Unpaired.out
	flagstat_end=`date +%s`
	...
	samtools sort -@ $NTHREADS -o $SORT_BAM_Unpaired $INIT_BAM_Unpaired
	sort_end=`date +%s`
	...
	# Creating stat file using samtools stats and measuring computation time
	samtools_stats_start=`date +%s`
	samtools stats -@ $NTHREADS $SORT_BAM > ${OUT_PATH_STAT}/${sample}/stats_stats.out
	samtools stats -@ $NTHREADS $SORT_BAM_Unpaired > ${OUT_PATH_STAT}/${sample}/stats_stats_Unpaired.out
	...
	samtools index $SORT_BAM_Unpaired
	index_end=`date +%s`
	...
	samtools idxstats $SORT_BAM_Unpaired > ${OUT_PATH_STAT}/${sample}/idxstat_stats_Unpaired.out
	idxstats_end=`date +%s`
