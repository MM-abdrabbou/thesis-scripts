...
REF_GENOME=/media/rna/INIA/benchmark/Alignment/Bowtie2/Reference_Bowtie2/bowtie.index
# Specifying the path containing the input data.
IN_PATH=/media/rna/INIA/benchmark/Data_Filtered
# Creaing and specifying the directory for the output alignment files bBAM
OUT_PATH_BAM=/media/rna/INIA/benchmark/Alignment/Bowtie2/Alignment_BAM
mkdir -p $OUT_PATH_BAM
# Creating and specifying the directory for the alignment statistics
OUT_PATH_STAT=/media/rna/INIA/benchmark/Alignment/Bowtie2/Alignment_stat
...
for folder in $IN_PATH/*; do
	...
	align_start=`date +%s`
	bowtie2 -x $REF_GENOME -1 $fastq1 -2 $fastq2  |samtools view  -bS - > $INIT_BAM
	align_end=`date +%s`
	align_time=$((align_end-align_start))
	echo "bowtie2 time in s:	$align_time" >> $COMP
